...
WIJZIGING TEN OPZICHTE VAN v1.0
-------------------------------
...
-------
    export BSD_KEY='...'
    python3 bouw_euromatches_v1.0.py              # lokaal, schrijft alleen
    python3 bouw_euromatches_v1.0.py --push       # pusht naar de repo
    python3 bouw_euromatches_v1.0.py --toon       # print de gevonden lijst
"""
...
def haal_events(league_id, season_id):
...
def push_naar_repo(pad, inhoud_bytes):
    ...
    payload = {
        'message': 'chore: update euromatches.json [skip ci]',
    ...
    try:
        ...
        print('[REPO] euromatches.json gepusht naar %s' % BADGES_REPO)
        return True
...
def main():
    ...
    print('=' * 66)
    print('bouw_euromatches v1.0 - CL, EL en Conference League')
    print('Verwacht seizoenslabel: %s' % SEIZOEN_LABEL)
    print('=' * 66)

    if not BSD_KEY:
        print('FOUT: BSD_KEY staat niet in de omgeving.')
        print('WIJZIGING=nee')
        return 1
    ...
    try:
        ...
        print('FOUT: clubs.json niet op te halen (%s)' % e)
        print('WIJZIGING=nee')
        return 1
    print('clubs.json: %d clubs als filter' % len(onze_clubs))
    ...
    for lid, (code, naam, sid) in LEAGUES.items():
        try:
            events, verwacht = haal_events(lid, sid)
        except Exception as e:
            print('  %-5s league %-3d FOUT: %s' % (code, lid, e))
            mislukt.append(code)
            continue
        ...
        print('  %-5s league %-3d season %-5s -> %3d events, %2d Belgisch%s'
    ...
    if mislukt:
        ...
        print('Niet alle competities opgehaald (%s). Niets weggeschreven.'
              % ', '.join(mislukt))
        print('WIJZIGING=nee')
        return 1

    print('Belgische wedstrijden gevonden: %d' % len(verse))

    if not verse:
        print('Geen Europese wedstrijden met een Belgische club.')
        print('Klopt het seizoenslabel of season_id niet meer, pas dan')
        print('SEIZOEN_LABEL en SEASON_IDS aan.')
        print('WIJZIGING=nee')
        return 0
